...
famd_raw <- read_csv2("flower2.CSV", locale = locale(encoding = "UTF-8")) |> 
  type_convert()

# Convert character
famd_raw[] <- lapply(famd_raw, function(x) {
  if (is.character(x) || (is.numeric(x) && length(unique(x)) <= 3)) as.factor(x) else x
})

# Separate species groups from morphological data
species_groups <- famd_raw$Sp.
famd_morpho <- famd_raw |> select(-Sp.)

# -------------------- 2. FAMD EXECUTION -------------------- #
res_famd <- FAMD(famd_morpho, graph = FALSE)
summary(res_famd)

# -------------------- 3. MULTIVARIATE HYPOTHESIS TESTING -------------------- #
# Gower distance matrix
dist_gower <- daisy(famd_morpho, metric = "gower")
...
contrib_table <- as.data.frame(res_famd$var$contrib) |> 
  rownames_to_column(var = "Trait") |> 
  mutate(Total_Contrib = Dim.1 + Dim.2) |> 
  arrange(desc(Total_Contrib))

head(contrib_table, 10)
...
plot_famd_clusters <- function(res_famd, res_hcpc, x_axis = 1, y_axis = 2) {
  ind_df <- as.data.frame(res_famd$ind$coord) |> 
    mutate(cluster = as.factor(res_hcpc$data.clust$clust),
      OTU = rownames(res_famd$ind$coord))
  colnames(ind_df)[x_axis] <- "AxisX"
  colnames(ind_df)[y_axis] <- "AxisY"
  hulls <- ind_df |> group_by(cluster) |> slice(chull(AxisX, AxisY))
...
# -------------------- 6. UPGMA DENDROGRAM -------------------- #
# Euclidean distance based on FAMD coordinates
dist_matrix <- dist(res_famd$ind$coord, method = "euclidean")
upgma_model <- hclust(dist_matrix, method = "average")

fviz_dend(upgma_model, k = 3,show_labels = TRUE, main = "UPGMA Dendrogram")