...
famd_raw <- read_csv2("mean_values.CSV", locale = locale(encoding = "UTF-8")) |> 
  type_convert()

species_groups <- famd_raw$Sp.[1:43] 
famd_morpho <- famd_raw[1:43, 2:23] |> mutate(L5 = log(L1 / L2), L6 = log(L3 / L4))
...
plot_famd_clusters <- function(res_famd, res_hcpc, x_axis = 1, y_axis = 2) {
  ...
  ggplot(ind_df, aes(x = AxisX, y = AxisY, color = cluster, shape = cluster)) +
    ...
    theme_classic() +
    labs(
      title = paste("FAMD - Dim", x_axis, "&", y_axis),
